...
package body Config is
   ...
   begin
      Cfg.Table := (others => (Key => Key_Reserved, Value => 0, Set => False)); Cfg.Level := None;
   end Init_Config;
   procedure Reset_Config (Cfg : in out Configuration) is begin Init_Config (Cfg); end Reset_Config;
   ...
   function Set_Value (Cfg : in out Configuration; Key : Config_Key; Value : Word32; Log : in out Errors.Error_Log) return Status_Code is
      Idx : Natural;
   begin
      if Key = Key_Reserved then Errors.Set_Error (Log, Configuration_Error, 0, "reserved"); return Configuration_Error; end if;
      if not Validate_Value (Key, Value) then Errors.Set_Error (Log, Out_Of_Range, 0, "range"); return Out_Of_Range; end if;
      Idx := Max_Config_Entries;
      for I in Cfg.Table'Range loop
         if Cfg.Table (I).Set and Cfg.Table (I).Key = Key then Idx := I; exit;
         elsif not Cfg.Table (I).Set and Idx = Max_Config_Entries then Idx := I; end if;
      end loop;
      if Idx = Max_Config_Entries then Errors.Set_Error (Log, Buffer_Overflow, 0, "full"); return Buffer_Overflow; end if;
      Cfg.Table (Idx) := (Key => Key, Value => Value, Set => True);
      if Key = Key_Auth_Level then
         case Value is when 0 => Cfg.Level := None; when 1 => Cfg.Level := Guest; when 2 => Cfg.Level := Operator; when 3 => Cfg.Level := Admin; when others => Cfg.Level := None; end case;
      end if;
      return Success;
   end Set_Value;
   ...
   begin
      Value := 0;
      for I in Cfg.Table'Range loop
         if Cfg.Table (I).Set and Cfg.Table (I).Key = Key then Value := Cfg.Table (I).Value; return Success; end if;
      end loop;
      return Configuration_Error;
   end Get_Value;
   function Is_Set (Cfg : Configuration; Key : Config_Key) return Boolean is
   begin
      for I in Cfg.Table'Range loop if Cfg.Table (I).Set and Cfg.Table (I).Key = Key then return True; end if; end loop;
      return False;
   end Is_Set;
   ...
   begin
      St := Get_Value (Cfg, Key_Max_Length, V); if St = Success then return Length_Type (V); else return 64; end if;
   end Get_Max_Length;
   function Get_Checksum_Seed (Cfg : Configuration) return Word16 is
      V : Word32; St : Status_Code;
   begin
      St := Get_Value (Cfg, Key_Checksum_Seed, V); if St = Success then return Word16 (V); else return 16#ADA1#; end if;
   end Get_Checksum_Seed;
   function Is_Strict (Cfg : Configuration) return Boolean is
      V : Word32; St : Status_Code;
   begin
      St := Get_Value (Cfg, Key_Enable_Strict, V); return St = Success and V = 1;
   end Is_Strict;
   ...
   function Count_Set_Entries (Cfg : Configuration) return Natural is
      C : Natural := 0;
   begin
      for I in Cfg.Table'Range loop if Cfg.Table (I).Set then C := C + 1; end if; end loop;
      return C;
   end Count_Set_Entries;
end Config;

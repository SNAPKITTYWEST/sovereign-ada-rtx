...
package Memory_Structures is
   ...
   function Append_Byte
     (Buf : in out Buffer;
      Value : Byte) return Status_Code;

   function Append_Bytes
     (Buf : in out Buffer;
      Data : Byte_Array) return Status_Code;

   function Get_Byte
     (Buf : Buffer;
      Index : Index_Type;
      Value : out Byte) return Status_Code;

   function Get_Slice
     (Buf : Buffer;
      Start : Index_Type;
      Count : Length_Type;
      Result : out Byte_Array) return Status_Code;
   ...
   function Insert_Item
     (Store : in out Data_Store;
      Item : Data_Item) return Status_Code;

   function Get_Item
     (Store : Data_Store;
      Index : Index_Type;
      Item : out Data_Item) return Status_Code;

   function Find_Item_By_Id
     (Store : Data_Store;
      Id : Word16;
      Item : out Data_Item;
      Found : out Boolean) return Status_Code;

...
package body Dispatcher is
   ...
   procedure Reset_Context (Ctx : in out Context) is begin Init_Context (Ctx); end Reset_Context;
   function Get_Current_State (Ctx : Context) return App_State is (State_Machine.Get_State (Ctx.SM));
   function Get_Config (Ctx : Context) return Config.Configuration is (Ctx.Cfg);
   function Get_Store (Ctx : Context) return Memory_Structures.Data_Store is (Ctx.Store);
   ...
   begin
      if Cmd.Arg_Count < 2 then Errors.Set_Error (Log, Invalid_Input, 0, "SET needs key value"); return Invalid_Input; end if;
      if Cmd.Args (0) > 7 then Errors.Set_Error (Log, Out_Of_Range, 0, "bad key"); return Out_Of_Range; end if;
   ...
   begin
      ...
      if Cmd.Arg_Count = 0 then Errors.Set_Error (Log, Invalid_Input, 0, "no data"); return Invalid_Input; end if;
      for I in 0 .. Cmd.Arg_Count - 1 loop Val := Math_Utils.Safe_Add (Val, Word32 (Cmd.Args (I)), Ov); end loop;
   ...
   begin
      ...
      if St = Success then Ada.Text_IO.Put_Line ("INTEGRITY=PASS"); else Ada.Text_IO.Put_Line ("INTEGRITY=FAIL"); end if;
      return State_Machine.Transition (Ctx.SM, Awaiting_Input, Log);
   end Handle_Verify;
   ...
   begin
      if Cmd.Arg_Count < 1 then Errors.Set_Error (Log, Invalid_Input, 0, "AUTH needs level"); return Invalid_Input; end if;
      case Cmd.Args (0) is
         when 0 => Lvl := None; when 1 => Lvl := Guest; when 2 => Lvl := Operator; when 3 => Lvl := Admin;
   ...
   begin Reset_Context (Ctx); return State_Machine.Transition (Ctx.SM, Idle, Log); end Handle_Reset;

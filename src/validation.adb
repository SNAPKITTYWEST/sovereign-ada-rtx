...
package body Validation is
   ...
   begin
      if Len > Max then return Buffer_Overflow; end if;
      return Success;
   end Validate_Buffer_Length;

   function Validate_Byte_Range (V : Integer) return Status_Code is
   begin
      if V < 0 or else V > 255 then return Out_Of_Range; end if;
      return Success;
   end Validate_Byte_Range;

   function Validate_Word16_Range (V : Integer) return Status_Code is
   begin
      if V < 0 or else V > 65535 then return Out_Of_Range; end if;
      return Success;
   end Validate_Word16_Range;

   function Validate_Word32_Range (V : Long_Long_Integer) return Status_Code is
   begin
      if V < 0 or else V > Long_Long_Integer (Word32'Last) then return Out_Of_Range; end if;
      return Success;
   end Validate_Word32_Range;

   function Validate_Data_Item (Item : Data_Item) return Status_Code is
   begin
      if not Item.Valid or else Item.Id = 0 then return Invalid_Input; end if;
      return Success;
   ...
   begin
      case From is
         ...
         when Processing =>
            if To = Serializing or To = Checking_Integrity or To = Authorized or To = Awaiting_Input or To = Error_State then return Success; end if;
   ...
   begin
      if Current < Level_Required then return Authorization_Denied; end if;
      return Success;
   end Validate_Auth_Required;

   procedure Validate_And_Set_Error (St : Status_Code; Log : in out Errors.Error_Log; Msg : String := "") is
   begin
      if St /= Success then Errors.Set_Error (Log, St, 0, Msg); end if;
   end Validate_And_Set_Error;
